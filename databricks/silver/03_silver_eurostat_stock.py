...
# MAGIC %md
# MAGIC # Silver — Eurostat Passenger Car Stock
# MAGIC
# MAGIC **Source table:** `bronze.car_stock_eurostat_raw`
# MAGIC **Target table:** `silver.car_stock_eurostat`
# MAGIC
...
# MAGIC | Add category | `fuel_category` — groups into Electric / Combustion / Hybrid / Total |
...
# MAGIC | Combustion | PET (petrol), DIE (diesel) |
# MAGIC | Hybrid | ELC_PET_HYB, ELC_DIE_HYB (non-plug-in hybrids) |
# MAGIC | Other | LPG, GAS, HYD_FCELL, etc. |
# MAGIC | Total | TOTAL |
...
df_silver = (
    ...
    .withColumnRenamed("value",    "stock_count")
    .select(
        "country_code",
        "year",
        "fuel_type_code",
...
# MAGIC %md
# MAGIC ## 5 — Romania: total fleet snapshot

# COMMAND ----------

print("Romania — total car stock by fuel category and year:")
display(
    df.filter(F.col("country_code") == "RO")
    .groupBy("year", "fuel_category")
    .agg(F.sum("stock_count").alias("total_stock"))
