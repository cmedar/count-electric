"""
Fetches the total stock of passenger cars by motor energy type from Eurostat
(dataset: ROAD_EQS_CARPDA) and lands the raw JSON response to
s3://count-electric/landing/raw/eurostat_stock/.

This is the stock equivalent of ROAD_EQR_CARPDA (new registrations).
"Stock" means all cars currently on the road — accumulated over all years,
not just cars sold in a given year.

Skips the upload if the fetched content is identical to the most recent
file already in S3 (compared by MD5 hash of canonical JSON).

Source: https://ec.europa.eu/eurostat/databrowser/view/ROAD_EQS_CARPDA/
Format: JSON-stat2
Cadence: Annual (published with ~12 month lag for the previous year)
"""
...
EUROSTAT_URL = (
    "https://ec.europa.eu/eurostat/api/dissemination/statistics/1.0/data/ROAD_EQS_CARPDA"
    "?format=JSON"
    "&freq=A"
    "&lang=EN"
)
...
def fetch_eurostat_data() -> dict:
    logger.info("Fetching Eurostat ROAD_EQS_CARPDA from API")
...
def main() -> None:
    ...
    upload_to_s3(s3, data, f"eurostat_road_eqs_carpda_{timestamp}.json")
    logger.info("Eurostat stock ingestion complete.")
